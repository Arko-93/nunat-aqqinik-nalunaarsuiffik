...
- **Shared coastline** — The V1 interim land/sea boundary is the complete OSM coastline land polygon surface (ODbL) unioned with Mapterhorn DEM land (elevation > ~1 m in a z12 coastal band, CC BY 4.0): the mask matches the land hillshade by construction, so no ocean layer can paint where the DEM renders land (issue #19). It drives the display mask above all ocean layers and is the reference shoreline for bathymetry clipping. Since issue #23, self-tiled IBCAO v5.2/GEBCO_2026 depth bands and contours are clipped to this same shoreline before tile generation (build-ocean-depth.py verifies its own output against the clip contract of web/src/map/coastline-clip.ts) — the display mask and the tiles cannot drift. Asiaq may replace both sources when authoritative distributable geometry arrives (then the licence section of the ocean-depth ATTRIBUTION must be updated too). Never use Natural Earth or partial landuse/landcover fills as the mask, and never let ocean layers paint above it.
