...
- Phase 5 UI: terrain-first map on `web/` (land hillshade + ocean meter bands hybrid D via `depth_abs_m` + discrete metric fills), V1 interim coastline land mask — OSM coastline ∪ Mapterhorn DEM land (ODbL + CC BY 4.0), above all ocean layers, map-first shell (search → Overview+Sources; no lens/Access/List–Map), KL/DA/EN switch (KL first; KL copy provisional), Download area corridor pack (OPFS; full Qaarsut→Kullorsuaq pack: land-relief z0–10 256 px + ocean-depth vector z0–12 + corridor mask z0–13 + localities, kind=full manifest, offline serves the same tile paths via OPFS; ocean hillshade raster not in pack), app-shell SW (`nunat-shell-*` only), not-for-navigation; gaps remain: Asiaq geometry swap for the shared shoreline, IBCAO self-tiles + clip-before-tile, ocean hillshade raster offline, land peak color bands, gap-label polish; no Google Fonts
