...
- Current data: 73 place identities, 73 classification assertions, 106 names, 15 geometries (58 imported localities have no Asiaq `geo_` yet), 73 memberships, 73 external identifiers (`nunagis.global_id`), 36 connections, and 37 service assertions
- Publication status: still not authoritative; `publish-check` reports **24** expected pending-provenance blockers (`src_legacy_seed` on 15 geometries, 3 Danish conflict names, 5 admin areas, 1 source row). Seed place status + memberships re-sourced to NunaGIS (54→24)
- Immediate work: Asiaq geometry parked (no external chase); Phase 6 regional AG air + RAL freight + Nuuk emergency air landed; settlement freight/SAR still open
- First operational extension: structural reachability (joins by `placeId`; multi-service export preserved)
- Selected release: `data/releases/CURRENT` → `2026.08.07.9` (web mounts via `scripts/sync-web-release.sh`; includes display-only NunaGIS `placenames.geojson`; isolation / single-dependency / seasonal-loss / freight-gap / emergency-gap reports; SQLite FTS5 `place_names_fts`)
- Read API: `api/` on `:8787` (`make api-dev`); place search uses FTS5 with LIKE fallback; OpenAPI client via `pnpm --dir api generate:client`; `/v1/reachability` supports `max_transfers`; `/v1/reports/capability-gap`
- Phase 5 UI: terrain-first map on `web/` (land hillshade + land peak color bands — peaks-only, transparent below 500 m, discrete 500/750/1000/1250/1500/2000/2500 m bands above the hillshade (raster-opacity 0.72 so form reads through) and below place labels, `nunat:land-peak-bands: 500-750-1000-1250-1500-2000-2500`; Liberty water relocated under the coastline mask so basemap water cannot wash islands; overview ocean fills darker @ 0.58 opacity with 50 m majors — + ocean meter bands hybrid D via `depth_abs_m` + discrete metric fills; ocean source is now self-tiled IBCAO v5.2 bathymetry with GEBCO_2026 fallback, depth bands/contours clipped to the shared coastline before tiling, raster hillshade + vector MVT served as same-origin PMTiles from `packages/ocean-depth`), V1 interim coastline land mask — OSM coastline ∪ Mapterhorn DEM land (ODbL + CC BY 4.0), above all ocean layers, map-first shell (search → Overview+Sources; no lens/Access/List–Map), KL/DA/EN switch (KL first; KL draft cleanup done, native review still open), skip-to-search (focuses search input), live search results region, WAI-ARIA dossier tabs + labelled article + mobile sheet focus trap, Download corridor pack (OPFS; status text when installed — no disabled “ready” button; OfflineStatus shows pack-none / stub / ready; full Qaarsut→Kullorsuaq pack: land-relief z0–10 native 512 px + land peak bands z0–10 + ocean-depth vector z0–11 + ocean hillshade raster z0–10 + corridor mask z0–13 + localities, kind=full manifest, offline serves the same tile paths via OPFS), app-shell SW (`nunat-shell-*` only), not-for-navigation, tile-gap labels (issue #26: quiet chrome chip when land DEM or ocean-depth tiles are absent in the viewport — protocol-reported, `nunat:tile-gap-labels: visible`, never a symbol layer so NunaGIS labels stay primary); gaps remain: Asiaq geometry swap for the shared shoreline; no Google Fonts; AccessPlanner deferred; native KL review
...
- Seed place status + 15 memberships re-sourced to `src_nunagis_placenames_register` (2026-08-07); geometries stay legacy until Asiaq; three Danish conflict names stay legacy; five `adm_` stay legacy until authority policy
