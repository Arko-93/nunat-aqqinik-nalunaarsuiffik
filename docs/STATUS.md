...
- Immediate work: Asiaq geometry parked (no external chase); next product tracks are API FTS / reachability dates / UI polish
- First operational extension: structural reachability (joins by `placeId`; multi-service export preserved)
- Selected release: `data/releases/CURRENT` → `2026.08.07.4` (web mounts via `scripts/sync-web-release.sh`; includes display-only NunaGIS `placenames.geojson`)
