...
- Phase 5 UI: terrain-first map on `web/` (land hillshade + ocean meter bands hybrid D), map-first shell (search → Overview+Sources; no lens/Access/List–Map), KL/DA/EN switch (KL first; KL copy provisional), Download area corridor pack (OPFS; stub pack until full PMTiles), not-for-navigation; no Google Fonts
