...
- Current data: 73 place identities, 73 classification assertions, 106 names, 15 geometries (58 imported localities have no Asiaq `geo_` yet), 73 memberships, 73 external identifiers (`nunagis.global_id`), 14 connections, and 14 service assertions
...
- Immediate work: Asiaq geometry parked (no external chase); Phase 6 graph growth + gap reports landed; more freight/emergency services still needed
- First operational extension: structural reachability (joins by `placeId`; multi-service export preserved)
- Selected release: `data/releases/CURRENT` → `2026.08.07.7` (web mounts via `scripts/sync-web-release.sh`; includes display-only NunaGIS `placenames.geojson`; isolation / single-dependency / seasonal-loss / freight-gap / emergency-gap reports; SQLite FTS5 `place_names_fts`)
